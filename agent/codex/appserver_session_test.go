...
// An MCP tool that documents its own call must surface that documentation, so
// the card shows "Fetch the design doc" rather than a blob of JSON arguments.
func TestAppServerSession_ItemStartedCarriesToolDescription(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()

	s := &appServerSession{events: make(chan core.Event, 4), ctx: ctx}
	s.handleItemStarted(map[string]any{
		...
		"arguments": map[string]any{
			"doc_id":      "doccnXXXX",
			"description": "Fetch the design doc",
		},
	})
	...
	if event.Type != core.EventToolUse {
		t.Fatalf("event type = %s, want %s", event.Type, core.EventToolUse)
	}
	if event.ToolDescription != "Fetch the design doc" {
		t.Fatalf("tool description = %q, want %q", event.ToolDescription, "Fetch the design doc")
	}
}

// A shell call has no description field in the Codex protocol; it must not be
// invented, so renderers stay on the command text.
func TestAppServerSession_ItemStartedLeavesShellDescriptionEmpty(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()

	s := &appServerSession{events: make(chan core.Event, 4), ctx: ctx}
	s.handleItemStarted(map[string]any{"type": "commandExecution", "command": "go build ./..."})
	...
	if event.ToolDescription != "" {
		t.Fatalf("tool description = %q, want empty", event.ToolDescription)
	}
}
